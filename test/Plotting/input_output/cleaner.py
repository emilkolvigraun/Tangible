...
# df = pd.read_csv('receiver_log.txt').sort_values(by='val').reset_index(drop=True)#, encoding = "UTF-16 LE")#
# print(df.head())
# df.to_csv('log_write_2.csv', index = True)
# exit(0)
...
df = pd.read_csv('log_write_2.csv')[:-5000]
x = []
y = []
h = []
start = df.iloc[0]['T0']
...
ax1.grid(True)
ax1.set_ylabel('Consumption frequency [Hertz]', fontsize=_font_size)
plt.sca(ax1)
plt.yticks(h_labels)
...
_min_x = min(x)
_max_x = max(x)
scater = ax0.scatter(x, y, alpha=.15)
scater.set_rasterized(True)
_y_avg_i, _y_labels, y_avg, y_min, y_max = calc(y, 3, 7, )
ax0.yaxis.set_label_position("right")
ax0.yaxis.tick_right()
ax0.set_xlabel('Time since start [ms]', fontsize=_font_size)
ax0.set_ylabel('Consumption interval [ms]', fontsize=_font_size)
plt.sca(ax0)
ax0.hlines((1/h_avg)*1000, _min_x, _max_x, colors=["r"])
plt.yticks(_y_labels)
plt.gca().get_yticklabels()[_y_avg_i].set_color("r")
plt.grid(True)
plt.xlim([_min_x, _max_x])

fig.tight_layout()
plt.show(block=False)
pdf = PdfPages('consumption_frequency.pdf')
pdf.savefig(fig)
plt.close()
pdf.close()