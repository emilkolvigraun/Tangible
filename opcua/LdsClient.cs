...
namespace EC.MS
{
    ...
    { 
        ...
        public KeyValuePair<bool, List<ApplicationDescription>> FindServers()
        {
            // use the LDS to find servers with the client configuration as input
            // endpoint transport profile uri is set to null
            try
            {
                return new KeyValuePair<bool, List<ApplicationDescription>>( 
                    true, 
                    LDS.FindServers(ClientConfiguration.GlobalDiscoveryServerUrl, null));
            }
        ...
        public List<string> FindServersUrl()
        {
            ...
            KeyValuePair<bool, List<ApplicationDescription>> servers = FindServers();
            
            if (servers.Key)
                servers.Value.ForEach(s => s.DiscoveryUrls.ForEach(d => urls.Add(d)));
        ...
        public List<string> FindServersAppName()
        {
            ...
            if (servers.Key)
                ...
                servers.Value.ForEach(s => names.Add(s.ApplicationName.ToString()));
        ...
        public Dictionary<string, List<string>> MapFindServers()
        {
            ...
            // foreach server and foreach discovery urls, add to the list of urls
            KeyValuePair<bool, List<ApplicationDescription>> foundServers = FindServers();

            if (foundServers.Key)
                // foreach server
                foundServers.Value.ForEach(s => {
