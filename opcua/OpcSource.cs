...
namespace EC.MS
{
    class OpcSource : ISource
    {
        ...
        public static OpcSource CreateOpcSource(string endpoint){
            try {
                DefaultLog.GetInstance().Log(LogLevel.INFO, string.Format("Creating OpcSource to: {0}", endpoint));
                return new OpcSource(Utils.GetUniqueKey(10), endpoint);
            }
            catch (Exception)
            {
                DefaultLog.GetInstance().Log(LogLevel.ERROR, string.Format("Failed to create OpcSource to: {0}", endpoint));
                return null;
            }
        }
            ...
            foreach(ReferenceDescription _ref in Client.BrowseObjectsNode())
            {
                if (_ref.BrowseName.ToString().Contains(nodeId) || _ref.NodeId.ToString().Contains(nodeId) || _ref.DisplayName.ToString().Contains(nodeId))
                {
                    Task.Run(() => {
                        ...
                        DefaultLog.GetInstance().Log(LogLevel.INFO, string.Format("Started subscribing to: {0}",_ref.DisplayName.ToString()));
                    });
                    return true;
                }
            }
        ...
        public bool Stop(string reference = null)
        {
            try 
            {
                if (reference == null) DefaultLog.GetInstance().Log(LogLevel.INFO, string.Format("Stopped ALL subscriptions to: {0}", Client.GetEndpoint().ToString()));
                ...
                if (reference == null || _ref.BrowseName.ToString().Contains(reference) || _ref.NodeId.ToString().Contains(reference))
                {
                    Task.Run(() => {
                        ...
                        DefaultLog.GetInstance().Log(LogLevel.INFO, string.Format("Stopped subscribing to: {0}",_ref.DisplayName.ToString()));
                    });
            ...
            { 
                DefaultLog.GetInstance().Log(LogLevel.ERROR, string.Format("Error in OpcSource: {0}", ex.Message));
            }
